...
void SsaBuilder::ComputeImmediateDom(BasicBlock** postOrder, int count)
{
    ...
    while (changed)
    {
        ...
        for (int i = count - 2; i >= 0; --i)
        {
            ...
            for (flowList* pred = m_pCompiler->BlockPredsWithEH(block); pred; pred = pred->getNext())
            {
                if (BitVecOps::IsMember(&m_visitedTraits, m_visited, pred->sourceBlock()->bbNum))
                {
                    predBlock = pred->sourceBlock();
                    break;
                }
            }
            ...
            for (flowList* pred = m_pCompiler->BlockPredsWithEH(block); pred; pred = pred->getNext())
            {
                if (predBlock != pred->sourceBlock())
                {
                    BasicBlock* domAncestor = IntersectDom(pred->sourceBlock(), bbIDom);
                    // The result may be NULL if "block" and "pred->sourceBlock()" are part of a
                    // cycle -- neither is guaranteed ordered wrt the other in reverse postorder,
                    // so we may be computing the IDom of "block" before the IDom of "pred->sourceBlock()" has
                    // been computed.  But that's OK -- if they're in a cycle, they share the same immediate
                    // dominator, so the contribution of "pred->sourceBlock()" is not necessary to compute
                    // the result.
...
void SsaBuilder::ComputeDominanceFrontiers(BasicBlock** postOrder, int count, BlkToBlkVectorMap* mapDF)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if ((blockPreds == nullptr) || (blockPreds->getNext() == nullptr))
        {
        ...
        for (flowList* pred = blockPreds; pred != nullptr; pred = pred->getNext())
        {
            DBG_SSA_JITDUMP("   Considering predecessor " FMT_BB ".\n", pred->sourceBlock()->bbNum);
            ...
            for (BasicBlock* b1 = pred->sourceBlock(); (b1 != nullptr) && (b1 != block->bbIDom); // !root && !loop
                 b1             = b1->bbIDom)
            {
